    ...
    <footer className="bg-background border-t border-border/40 pt-16 pb-8">
      <div className="container mx-auto px-4 md:px-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
          <div className="space-y-6">
            ...
            <p className="text-sm text-muted-foreground leading-relaxed">
              Your trusted partner for end-to-end technology services, security solutions, and digital transformation in Nepal.
            </p>
            <div className="flex gap-4">
              {[Facebook, Twitter, Linkedin].map((Icon, i) => (
                <Link key={i} href="#" className="h-10 w-10 rounded-full bg-muted flex items-center justify-center hover:bg-primary hover:text-primary-foreground transition-colors">
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm text-muted-foreground">
              {["Software Development", "CCTV Surveillance", "Fire Safety", "Cloud Solutions", "Power Backup"].map((item) => (
                <li key={item}>
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm text-muted-foreground">
              {["About Us", "Our Clients", "Contact", "Privacy Policy"].map((item) => (
                <li key={item}>
                  <Link href="#" className="hover:text-primary transition-colors flex items-center gap-2 group">
                    <ArrowRight className="h-3 w-3 opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all" />
                    {item}
        ...
        <div className="border-t border-border/40 pt-8 text-center text-sm text-muted-foreground">
          <p>&copy; {new Date().getFullYear()} DeskPro Technology Pvt. Ltd. All rights reserved.</p>
        </div>
      </div>
    </footer>
  )
